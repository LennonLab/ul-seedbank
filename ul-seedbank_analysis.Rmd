...
# Figure 4
```{r}
alpha.div.total <- rowSums(OTUs.total.pa) 
alpha.div.active <- rowSums(OTUs.active.pa) 
...
ggplot(joint_data, aes(x = abund, y = lambda)) + 
  ...
  theme(legend.position = "none", axis.title = element_text(size = 40),
        strip.text = element_text(size = 32), title = element_text(size = 32),
        axis.text = element_text(size = 24)) + 
  scale_x_log10(labels = c(1e-4, 1e-2, 1), breaks = c(1e-4, 1e-2, 1)) +
  facet_wrap(~OTU, nrow = 9) +
  labs(x = "Relative abundance at time t",
       y = expression(Ln(N[t+1]/N[t])),
       title = "NFD for persistent OTUs when Active") + 
  ggsave("figures/figS3.pdf", height = 22.5*1.5, width = 30*1.5) +
  ggsave("figures/figS3.png", height = 22.5*1.5, width = 30*1.5, dpi = 500)
...
cbind.data.frame(env.vars['temp'], decostand(OTUs.active.persistent, method = "total")[,sample(1:ncol(OTUs.active.persistent),25)]) %>% 
  ...
  geom_line(stat = "smooth", method = "lm", formula = y ~ x + I(x^2), se=F, span = 0.5, alpha = 0.8, size = 1) +
  ...
  theme(legend.position = "none", axis.title = element_text(size = 32),
        strip.text = element_text(size = 24), title = element_text(size = 24),
