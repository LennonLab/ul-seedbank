...
null_dist_toplot_ses %>% 
  ...
  ggsave("figures/figS3.pdf", width = 6, height = 3/4*6)

merge(reactivations, select(null_dist_toplot_ses, OTU, taxon_ses)) %>% 
  mutate(sig = ifelse((taxon_ses) < -2, "Environmentally cued", "Stochastic")) %>% 
  mutate(sig = ifelse(is.na(sig), "Indeterminate", sig)) %>% 
  mutate(sig = factor(sig, levels = c("Stochastic", "Environmentally cued", "Indeterminate"))) %>% 
  ggplot(aes(y = times_activated, x = relabund)) + 
  geom_point(aes(color = sig), size = 3, alpha = 0.5) + 
  geom_line(data = augment(exp.mod), aes(x = relabund, y = .fitted), size = 1) + 
  #geom_smooth(color = "black", span = 1) + 
  #facet_wrap(~subset, ncol = 1) +
...
null_dist_toplot %>% 
  ...
  geom_point(data = subset(null_dist_toplot, obsim == "Simulated"), size = 1, alpha = 0.05) +
...
# create a graph of standardized effect sizes for each point, relative to the overall relabund (not the avg. of sites), which should be kept constant by the shuffling algorithm. So there should be 82 OTUs and 82 SES values 
```
