...
env.data %>% select(date, temp, spc, oxygen, salinity, secchi, ph, chla, tp, tn, doc) %>% gather(-date, key = "variable", value = "val") %>% 
...
tree_plot <- tree_annotated + 
  ...
  geom_tippoint(aes(color = lubridate::month(date, label = T))) +
  scale_color_manual(values = shifter(darken(plasma(12, direction = -1),.1), 9)) + 
