...
ggplot(data = env.tsmod) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none") +
  ggsave("figures/fig4_arima.pdf", bg = "white", height = 3/4*6, width = 6)
...
# different null model to test against, here just proportional drift
null.stochastic <- function(comm){
  comm.rel <- decostand(comm, method = "total")
  comm.null <- comm.rel * 0
  comm.proportions <- colMeans(comm.rel)
  for(r in 1:nrow(comm.null)){
    abunds.i <- sample(x = 1:ncol(comm.null), 
                       size = sum(comm[r,]),
                       prob = comm.rel[r,],
                       replace = TRUE)
    abunds.i <- factor(abunds.i, levels = as.character(1:ncol(comm.null)), ordered = T)
    table.i <- table(abunds.i)
    table.i[table.i==0] <- 0.5
    comm.null[r,] <- table.i
  }
  return(comm.null)
}
...
ggplot(joint_data, aes(x = abund, y = lambda)) + 
  ...
  ggsave("figures/NFD_individual_OTUs_active.pdf", height = 20, width = 30)
...
plot_grid(nfdactiveslopesfig, nfdactivefig, ncol = 1, labels = "AUTO") +
  ggsave("figures/figS1.pdf", width = 6, height = 8)
...
ggplot(joint_data, aes(x = abund, y = lambda)) + 
  ...
  labs(x = "Relative abundance at time t",
       y = expression(Ln(N[t+1]/N[t]))) + 
  ggsave("figures/NFD_individual_OTUs_total.pdf", height = 20, width = 30)
...
# iternum <- 5000
# 
# sensitivity_analysis_output <- data.frame(NULL)
# sorted_taxa <- names(sort(colMeans(OTUs.total.nozero)))
# for(i in seq(3, ncol(OTUs.total.nozero), by = 4)){
#   
#   # calculate total 
#   OTUs.total.subset <- OTUs.total.nozero[,sorted_taxa[1:i]]
#   tot.nd <- foreach(i=1:iternum, .combine=rbind, .packages = c("vegan","tidyverse")) %dopar% {
#      get.cov(OTUs.total.subset)
#   }
#   obs.nfd <- calc.neg.freq.dep(OTUs.total.subset) %>% 
#     filter(-fd > 0)  
#   obs.cov_total <- cov(log(obs.nfd$eq.freq), log(-obs.nfd$fd))
#   total_df <- cbind.data.frame(N_tax = i, 
#         N_sim = iternum,
#         subset = "total",
#         null_sd = sd(tot.nd), 
#         null_min = min(tot.nd), 
#         null_max = max(tot.nd),
#         null_mean = mean(tot.nd),
#         obs_cov = obs.cov_total)
#         
#   
#   # calculate active
#   OTUs.active.subset <- OTUs.active.nozero[,sorted_taxa[1:i]]
#   act.nd <- foreach(i=1:iternum, .combine=rbind, .packages = c("vegan","tidyverse")) %dopar% {
#      get.cov(OTUs.active.subset)
#   }
#   obs.nfd <- calc.neg.freq.dep(OTUs.active.subset) %>% 
#     filter(-fd > 0)  
#   obs.cov_active <- cov(log(obs.nfd$eq.freq), log(-obs.nfd$fd))
#   active_df <- cbind.data.frame(N_tax = i, 
#         N_sim = iternum,
#         subset = "active",
#         null_sd = sd(act.nd), 
#         null_min = min(act.nd), 
#         null_max = max(act.nd),
#         null_mean = mean(act.nd),
#         obs_cov = obs.cov_active)
#   
#   i_out <- rbind.data.frame(active_df, total_df)
#   sensitivity_analysis_output <- rbind.data.frame(sensitivity_analysis_output, i_out)
#   
# }
# 
# #stop cluster
# stopCluster(cl)
# 
# write_csv(sensitivity_analysis_output, "analysis/sensitivity_analysis.csv")

sensitivity_analysis_output <- read_csv("analysis/sensitivity_analysis.csv")

sensitivity_analysis_output %>% 
  ggplot(aes(x = N_tax, y = obs_cov/null_mean, color = subset)) + 
  geom_point() + 
  geom_line()

sensitivity_analysis_output %>% 
  ggplot(aes(x = N_tax, 
             y = (obs_cov - null_mean)/null_sd, 
             color = subset, 
             shape = subset)) + 
  geom_hline(yintercept = c(2,-2), alpha = 0.2, linetype = "dashed") +
  geom_point(alpha = .8, size = 2) + 
  annotate("text", x = 85, y = -2.5, label = "Total", size = 4.5, color = "black") +
  annotate("text", x = 85, y = -5.5, label = "Active", size = 4.5, color = "black") +
  geom_line() +
  theme(legend.position = "none") +
  ...
  ggsave("figures/sensitivity_analysis_SES.pdf", width = 6, height = 5)

sensitivity_analysis_output %>% 
  ggplot(aes(x = N_tax, y = obs_cov, color = subset)) + 
  geom_point(alpha = .8, size = 2) + 
  annotate("text", x = 60, y = -.3, label = "Total", size = 4.5, color = "black") +
  annotate("text", x = 20, y = -.7, label = "Active", size = 4.5, color = "black") +
  geom_line() +
  theme(legend.position = "none") +
  scale_color_discrete() +
  labs(x = "Number of taxa", 
       y = "Observed Cov(NFD, Eq. Freq.)") + 
  ggsave("figures/sensitivity_analysis_obsCov.pdf", width = 6, height = 5)
...
otu.total.relabund2 <- colMeans(decostand((OTUs.total), method = "total"))
otu.active.relabund2 <- colMeans(decostand((OTUs.active), method = "total"))
...
cbind.data.frame(OTU = colnames(OTUs.total),
  ...
  scale_x_log10() +
  ggsave("figures/fig5a.pdf", width = 6, height = 3/4*6)
...
obs_corr <- cor(obs_dat$times_activated, obs_dat$relabund)
...
```{r}
shuffle.ts.total <- function(total.spec.ts){
  ts.length = length((total.spec.ts)) / 2
  sample.index = sample(1:ts.length)
  return(total.spec.ts[c(sample.index, (sample.index + ts.length))])
}

reap.nullmod <- function(DNAcomm = OTUs.total,
                         RNAcomm = OTUs.active,
  ...
  DNAsims <- as.array(simulate(DNAnullcom, nsim = n))
  print("Generating RNA null models.")
  RNAsims <- as.array(simulate(RNAnullcom, nsim = n))
...
# # Run Sims
# null_dist_tot <- list(corr = vector(), dat = data.frame()) # <- reap.nullmod(n = 100, algorithm = "c0_ind")
# 
# nsims = 1000
# while(nsims > 0){
#   # run this batch of sims
#   sims_to_run <- min(100, nsims)
#   null_dist_i <- reap.nullmod(n = sims_to_run, algorithm = "c0_ind")
#   
...
#saveRDS(null_dist_tot, "analysis/null_dists_reactivations.rda")
null_dist_tot <- readRDS("analysis/null_dists_reactivations.rda")
# analyze null dist
hist(null_dist_tot$corr, xlim = c(-.35,-.1))
abline(v = obs_corr)
...
null_dist_toplot %>% 
  ggplot(aes(x = relabund, y = times_activated, group = sim, color = obsim)) + 
  geom_jitter(data = subset(null_dist_toplot, obsim == "Simulated"), size = 1, alpha = 0.05) +
  geom_line(data = subset(null_dist_toplot, obsim == "Simulated"), size = 1, alpha = 0.2, stat = "smooth", span = .5, method = "loess", se = F) +
  geom_point(data = subset(null_dist_toplot, obsim == "Observed"), size = 3, alpha = 0.5) + 
  geom_line(data = subset(null_dist_toplot, obsim == "Observed"), size = 1, alpha = 1, stat = "smooth", span = 1, method = "loess", se = F) +
  theme(legend.position = c(.85, .85), legend.text = element_text(size = 10)) +
  ylab("Reactivations from seed bank") +
  xlab("Average relative abundance") +
  scale_x_log10() + 
  scale_color_manual(values = c("black", "gray70")) +
  #scale_alpha_discrete(range = c(.8, 0.1)) + 
  ggsave("figures/fig5.pdf", width = 6, height = 3/4*6) +
  ggsave("figures/fig5.png", width = 6, height = 3/4*6, dpi = 500)
...
env.pcs.subset %>% 
  ...
  geom_text_repel(data = as.data.frame(env.pca$rotation), 
            ...
            segment.alpha = 0) +
  coord_fixed() +
  ggsave("figures/figS2.pdf", bg = "white", width = 8, height = 8)
...
cbind.data.frame(env.vars['temp'], decostand(OTUs.active.persistent, method = "total")[,sample(1:ncol(OTUs.active.persistent),25)]) %>% 
  ...
  ggplot(aes(x = Value, y = log10(Abundance), color = OTU)) + 
  geom_point(alpha = 0.3) + 
  geom_line(stat = "smooth", method = "lm", formula = y ~ x + I(x^2), se=F, span = 0.5, alpha = 0.5) +
...
set.seed(123)
cbind.data.frame(env.vars['spc'], decostand(OTUs.active.persistent, method = "total")[,sample(1:ncol(OTUs.active.persistent),25)]) %>% 
  gather(-spc, key = "OTU", value = "Abundance") %>% 
  gather(spc, key = "Variable", value = "Value") %>% 
