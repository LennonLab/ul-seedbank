...
data.frame(date = env.data$date, Total = alpha.div.total, Active = alpha.div.active) %>% 
  ...
  geom_point(aes(color = lubridate::month(date, label = T)), size = 3, alpha = 0.7) +
  ...
  ylab("Richness ratio (total : active)") +
  xlab("") +
...
env.data %>% select(date, temp, spc, oxygen, salinity, secchi, ph, chla, tp, tn, doc) %>% 
  rename("Chl a (mg/L)" = chla,
...
persistent.grs.all %>%   
  ...
  #scale_y_continuous(limits = c(-5, 12)) +
   ggsave("figures/fig2.pdf", bg = "white", width = 6, height = 6)
