...
fig.pca <- full_join(design[which(design$sample.type == "RNA"),], active.traj.df) %>%
  ...
  geom_path(alpha = 0.4, arrow = arrow(angle = 15, length = unit(0.1, "inches"),
                  ends = "last", type = "closed")) + 
  geom_point(aes(color = lubridate::month(date, label = T)), alpha = 0.6, size = 2) +
...
fig.rda <- ggplot() + 
  ...
  geom_point(data = active.site.scores, alpha = 0.3,
            aes(x = RDA1, y = RDA2, 
  ...
  ggsave("figures/rda_active_aem.pdf", bg = "white", height = 6, width = 8) + 
  ggsave("figures/rda_active_aem.png", bg = "white", height = 6, width = 8, dpi = 600)

plot_grid(fig.pca, fig.rda + theme(legend.position = "none"), align = "hv", axis = "lr", ncol = 2, labels = "AUTO") +
  ggsave("figures/ordinations_active.png", width = 12, height = 3/8*16, dpi = 600)
