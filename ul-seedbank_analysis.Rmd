...
reap.nullmod <- function(comm = OTUs, samp.design = design, n = 10){
  
  null_correlations <- numeric(length = n)
  # loop through null models
  for(i in 1:n){
    
    # generate null model and separate into corresponding DNA and RNA sets
    nullcom <- apply(comm, MARGIN = 2, FUN = shuffle.ts.total)
    nullcom_active <- nullcom[which(samp.design$sample.type == "RNA"),]
    nullcom_total <- nullcom[which(samp.design$sample.type == "DNA"),]
