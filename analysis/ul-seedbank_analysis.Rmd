...
# weather.data <- read.csv("data/noaa-climate.csv")
# weather.bloomington <- weather.data %>% 
#   filter(NAME == "BLOOMINGTON INDIANA UNIVERSITY, IN US") %>%
#   select(STATION, DATE, PRCP, SNOW) %>% mutate(Date = as_date(DATE)) %>%
#   filter(Date > min(env.data$date) & Date < max(env.data$date))
# 
# btown.precip <- as.xts(weather.bloomington$PRCP, order.by = weather.bloomington$Date)
# plot(btown.precip)
# weekly.precip <- apply.weekly(btown.precip, sum)
# 
# plot(weekly.precip)
# ggplot(, aes(y = PRCP, x = Date)) +
#   geom_line() + 
#   ylab("Precipitation")
# na.approx(coredata(weekly.precip), xout = env.data$date)
```
