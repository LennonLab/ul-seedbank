...
### Is temporal beta diversity driven by nestedness or turnover?
```{r}
...
ddr.env <- timeddr %>% gather(key = Community, value = Dissimilarity, -EnvDist, -Time) %>% 
  ggplot(aes(x = EnvDist, y = (1-Dissimilarity), color = Community)) + 
  geom_point(alpha = 0.1, shape = 16, size = 0.7) +
  geom_smooth(size = 2, method = "gam") +
  scale_y_log10() +
  labs(y = "Community Similarity", x = "Environmental Dissimilarity") +
...
ddr.time <- timeddr %>% gather(key = Community, value = Dissimilarity, -EnvDist, -Time) %>% 
  ggplot(aes(x = Time, y = 1-Dissimilarity, color = Community)) + 
