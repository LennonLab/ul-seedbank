...
OTUs.total.persistent <- OTUs.ranked[,which(
  colSums(decostand(OTUs.ranked, method = "pa")) >= 0.5 * nrow(OTUs.ranked))]
...
nfd.response <- function(OTUs = NULL, OTUs.active = NULL, num.seq = 10, num.iter = 999, plotit = F, incidence.freq = 0.5, max.taxa = NULL){
  
  # separate OTUs present across the whole time series
  OTUs.ranked <- OTUs[,names(sort(colSums(OTUs), decreasing = T))]
  OTUs.persistent <- OTUs.ranked[,which(
    colSums(decostand(OTUs.ranked, method = "pa")) >= incidence.freq * nrow(OTUs.ranked))]
  ...
  max.taxa <- floor(seq(5, min(max.taxa, ncol(OTUs.persistent)), length.out = num.seq))
  output <- data.frame(taxa = max.taxa, 
             p = numeric(length(max.taxa)), 
             ses = numeric(length(max.taxa)),
             strength = numeric(length(max.taxa)))
...
active.nfd.response <- nfd.response(OTUs = OTUs.total, OTUs.active = OTUs.active, num.seq = 10, num.iter = 999, max.taxa = 100)
plot(strength ~ taxa, active.nfd.response)
plot(p ~ taxa, active.nfd.response)

total.nfd.response <- nfd.response(OTUs = OTUs.total, OTUs.active = NULL, num.seq = 5, num.iter = 100, max.taxa = 100)
plot(strength ~ taxa, total.nfd.response)
plot(p ~ taxa, total.nfd.response)

ncol(OTUs.total.nozero)
taxavec <- c(3,5,8, 10, 20, 25)
total.nfd.response <- nfd.response(OTUs = OTUs.total, max.taxa = taxavec, num.iter = 999)
```
