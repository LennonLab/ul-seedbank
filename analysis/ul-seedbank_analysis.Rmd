...
apply(total.traj, 2, cv)
apply(active.traj, 2, cv)
```
...
# # remove transients
# seedbank.pa <- seedbank.pa[, -which(colnames(seedbank.pa) %in% colnames(transients))]
# 
...
# 
# 
# pc1.pa.dyn <- rbind.data.frame(
#   cbind.data.frame(total.pa.traj.df, subset = "total.pa", Date = env.data$date),
#   cbind.data.frame(active.pa.traj.df, subset = "active.pa", Date = env.data$date)) %>% 
#   ggplot(aes(x = Date, y = Comp.1, color = subset)) + 
#   geom_line(size = 0.75) + 
#   scale_color_manual("Community Subset", values = my.cols[c(1,4)]) + 
#   ylab(paste("PCoA 1 (",var1,"%)", sep = "")) + 
...
ddr.env <- timeddr %>% gather(key = Community, value = Dissimilarity, -EnvDist, -Time) %>% 
  ggplot(aes(x = EnvDist, y = (Dissimilarity), color = Community)) + 
  geom_point(alpha = 0.1, shape = 16, size = 0.7) +
  geom_smooth(size = 2) +
  #scale_y_log10() +
  labs(y = "Community Dissimilarity", x = "Environmental Dissimilarity") +
  scale_color_manual(values = my.cols[c(4,2,1,3)]) +
  ggsave("figures/dissimilarity-by-env.pdf", bg = "white", height = 8, width = 8)
ddr.env

ddr.time <- timeddr %>% gather(key = Community, value = Dissimilarity, -EnvDist, -Time) %>% 
  ggplot(aes(x = Time, y = Dissimilarity, color = Community)) + 
...
active.rda <- rda(OTUs.rel.act.subs ~ AEM5 + AEM4 + AEM6 + AEM2 + temp + doc + oxygen + spc + ph + secchi, data = cbind.data.frame(env.preds, aem.subs))
...
active.spec.scores <- spec.scores[which(spec.scores$RDA1 > 0.25 | spec.scores$RDA2 > 0.25),] %>% 
...
ggplot() + 
  ...
  geom_label_repel(data = active.spec.scores, 
               aes(x = RDA1, y = RDA2), 
               label = active.spec.scores$Genus) + 
...
total.rda <- rda(OTUs.rel.tot.subs ~ AEM1 + AEM2 + AEM4 + AEM5 + AEM6 + AEM10 + temp + doc + oxygen + spc + ph + secchi, data = cbind.data.frame(env.preds, aem.subs))
...
total.spec.scores <- spec.scores[which(spec.scores$RDA1 > 0.25 | spec.scores$RDA2 > 0.25),] %>% 
  rownames_to_column(var = "OTU") %>% inner_join(OTUs.tax)
...
ggplot() + 
  ...
  theme_cowplot() + 
  coord_fixed() + 
  ggsave("figures/rda_total_aem.pdf", bg = "white", height = 8, width = 8)
...
nfd.nulldists %>% 
  ggplot(aes(y = nfd, x = subset, fill = subset)) + 
...
ggplot() +
  geom_jitter(data = env.pcs.subset, 
              aes(x = PC1, y = PC2, size = rate, color = date),
              alpha = 0.25) +
