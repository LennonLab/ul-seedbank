...
OTUs.bray.tot <- vegdist(OTUs.total, method = "bray")
OTUs.bray.act <- vegdist(OTUs.active, method = "bray")
...
timeddr %>% gather(key = Community, value = Dissimilarity, -EnvDist, -Time) %>% 
...
timeddr <- data.frame(total = as.vector(OTUs.bray.tot),
           ...
           seedbank = as.vector(OTUs.bray.sb),
...
lib <- c(1,52)
pred <- c(53,123)
s.out <- simplex(limno, lib, pred)
str(s.out)
plot(s.out$E, s.out$rho, type = 'l')
s.out <- simplex(limno, lib, pred, E = 10, tp = 1:10)
plot(s.out$tp, s.out$rho, type = 'l')
s.map.out <- s_map(limno, lib, pred, E = 2)
plot(s.map.out$theta, s.map.out$rho, type = 'l',
     xlab = "nonlinearity (theta)", ylab = "rho")
```
...
library(AEM)
samplingdays <- env.data$date
time.dist <- numeric(length(samplingdays))
intervals <- as.vector(diff.Date(samplingdays))
w <- 1/(intervals/max(intervals))
aem.out <- aem.time(n = length(samplingdays),
                    w = w, moran = T,
                    plot.moran = T)
aem.eigs <- aem.out$aem
aem.out$values
total.varpar <- varpart(OTUs.total, na.omit(env.vars), aem.eigs, transfo = "hellinger")
total.rda <- rda(OTUs.total, aem.eigs)
...
```{r}
OTUs.total.persistent <- OTUs.ranked[,which(
  colMeans(decostand(OTUs.ranked, method = "pa")) >= .9)]
OTUs.active.persistent <- OTUs.active[,colnames(OTUs.total.persistent)]
total.persistent <- as.data.frame(OTUs.total.persistent)
active.persistent <- as.data.frame(OTUs.active.persistent)
total.persistent$date <- env.data$date
active.persistent$date <- env.data$date
total.persistent %>%
  gather(key = OTU, value = abundance, -date) %>% 
  ggplot(aes(x = date, y = abundance, color = OTU)) + 
  geom_line() +
  scale_y_log10()
active.persistent %>%
  gather(key = OTU, value = abundance, -date) %>% 
...
inactive <- which(colMeans(decostand(OTUs.active.persistent, method = 'pa')) < 1)
matplot(OTUs.active.persistent[,inactive], type = 'l')


calc.growth <- function(sbs, dates){
  y.s <- matrix(ncol = ncol(sbs), nrow = nrow(sbs)-1)
  dt <- as.vector(diff(dates))
  for(i in 1:(nrow(sbs)-1)){
    y.s[i,] = (log(sbs[i+1,]) - log(sbs[i,])) / dt[i]
  }
  
  return(y.s)
}
active.growths <- calc.growth(sbs = (OTUs.active.persistent + 1), dates = env.data$date)
plot(active.growths[,2], type = 'l')
max.growths <- apply(active.growths, MARGIN = 2, FUN = which.max)
data.frame(date = month(env.data$date[max.growths], label = T)) %>% 
  ggplot(aes(x = date)) + 
  geom_bar()
dim(active.growths)
dim(OTUs.active.persistent[-1,])

total.growths <- calc.growth(sbs = (OTUs.total.persistent + 1), dates = env.data$date)
plot(total.growths[,2], type = 'l')
max.growths <- apply(active.growths, MARGIN = 2, FUN = which.max)
data.frame(date = month(env.data$date[max.growths], label = T)) %>% 
  ggplot(aes(x = date)) + 
  geom_bar()
